...
class TemplatesCourseDetailRDFaCMSTestCase(CMSTestCase):
    ...
    def test_templates_course_detail_rdfa(self):
        ...
        main_organization = OrganizationFactory(page_title="Main org", fill_logo=True, should_publish=True)
        other_organization = OrganizationFactory(page_title="Other org", fill_logo=True, should_publish=True)
        
        # Create persons
        author1 = PersonFactory(page_title="François", fill_portrait=True)
        placeholder = author1.extended_object.placeholders.get(
            slot="bio"
        )
        add_plugin(
            language="en",
        ...
        author2 = PersonFactory(page_title="Jeanne", fill_portrait=True, should_publish=True)
        ...
        dom = parser.parse(io.StringIO(str(response.content)))
        ...
        self.assertTrue(
            (subject, URIRef("https://schema.org/isAccessibleForFree"), Literal("true"))
            in graph
        )
        self.assertTrue(
        ...
        logo_value, = graph.objects(URIRef('/en/main-org/'), URIRef("https://schema.org/logo"))
        pattern = (
        ...
        logo_value, = graph.objects(URIRef('/en/other-org/'), URIRef("https://schema.org/logo"))
        self.assertIsNotNone(re.search(pattern, str(logo_value)))
        ...
        for author_subject in author_subjects:
            portrait_value, = graph.objects(author_subject, URIRef("https://schema.org/image"))
